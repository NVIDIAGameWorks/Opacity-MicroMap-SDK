...
namespace raster
{
	PRECISE float4 TexCoord_to_VS_SV_Position(PRECISE float2 texCoord)
	{
		...
		const PRECISE float2 uv = (texCoord * g_GlobalConstants.TexSize + g_GlobalConstants.ViewportOffset) * g_GlobalConstants.InvViewportSize;
		return float4(float(uv.x) * 2 - 1, 1 - float(uv.y) * 2, 0, 1);
	}

	PRECISE float2 PS_SV_Position_to_TexCoord(PRECISE float4 svPosition)
	{
		TextureFilterMode mode = (TextureFilterMode)g_GlobalConstants.FilterType;

		PRECISE float2 uv = (svPosition.xy - g_GlobalConstants.ViewportOffset) * g_GlobalConstants.InvTexSize;
		const PRECISE float2 halfTexel = 0.5 * g_GlobalConstants.InvTexSize;
		PRECISE float2 texCoord = mode == TextureFilterMode::Linear ? uv - halfTexel : uv;
		return texCoord;
	}
...
#define ENABLE_EXACT_CLASSIFICATION     (0) // The exact classification is expensive. This should only be enabled for debug purposes.
    ...
    void GetStateAtPixelCenter(PRECISE float2 texCoord, PRECISE float2 texel, PRECISE float2 i_verts0, PRECISE float2 i_verts1, PRECISE float2 i_verts2, uint i_primitiveId, inout bool IsOpaque, inout bool IsTransparent)
    {
        ...
        {
            const PRECISE float alpha = t_alphaTexture.SampleLevel(s_samplers[g_GlobalConstants.SamplerIndex], texCoord.xy, 0).w;
