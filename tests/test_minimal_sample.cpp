...
namespace {
	...
	TEST(MinimalSample_Serialize, CPU)
	{
		// This sample will demonstrate the use of OMMs on a triangle fan modeled on top of a donut.
		const float rMin = 0.2f; // Circle inner radius.
		const float rMax = 0.3f; // Circle outer radius.
		...
		ommBlob blob = 0;
		res = omm::Cpu::Serialize(bakerHandle, &bakeDesc, nullptr, &blob);
		ASSERT_EQ(res, omm::Result::SUCCESS);

		const omm::Cpu::BlobDesc* blobDesc = nullptr;
		res = omm::Cpu::GetBlobDesc(blob, &blobDesc);
		ASSERT_EQ(res, omm::Result::SUCCESS);
		ASSERT_NE(blobDesc, nullptr);

		ommBlob blobCopy = 0;
		res = omm::Cpu::Deserialize(bakerHandle, blobDesc, &blobCopy);
		ASSERT_EQ(res, omm::Result::SUCCESS);

		const omm::Cpu::BakeInputDesc* bakeDescCopy = nullptr;
		res = omm::Cpu::GetBlobInputDesc(blobCopy, &bakeDescCopy);
		ASSERT_EQ(res, omm::Result::SUCCESS);
		ASSERT_NE(bakeDescCopy, nullptr);

		ASSERT_EQ(bakeDescCopy->bakeFlags, bakeDesc.bakeFlags);
		ASSERT_EQ(bakeDescCopy->runtimeSamplerDesc.addressingMode, bakeDesc.runtimeSamplerDesc.addressingMode);
		ASSERT_EQ(bakeDescCopy->runtimeSamplerDesc.filter, bakeDesc.runtimeSamplerDesc.filter);
		ASSERT_EQ(bakeDescCopy->runtimeSamplerDesc.borderAlpha, bakeDesc.runtimeSamplerDesc.borderAlpha);

		// perform the baking... processing time may vary depending on triangle count, triangle size, subdivision level and texture size.
		omm::Cpu::BakeResult bakeResultHandle;
		res = omm::Cpu::Bake(bakerHandle, *bakeDescCopy, &bakeResultHandle);
		ASSERT_EQ(res, omm::Result::SUCCESS);
