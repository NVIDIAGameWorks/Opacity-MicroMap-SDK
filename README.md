...
Micro-Maps.
- See [NVIDIA OptiX](https://developer.nvidia.com/designworks/optix/download) and the [NVIDIA OptiX Programming Guide](https://raytracing-docs.nvidia.com/optix7/guide/index.html#acceleration_structures#accelstruct-omm) goes in to more detail on the internal layout of the Opacity Micro-Map format.
# Build Instructions

## Windows

1. Clone the repository with all submodules:
	- `git clone --recursive https://github.com/NVIDIAGameWorks/omm-bake-sdk.git`
...
3. Assuming that the OMM Bake SDK tree is located in `D:\omm-bake-sdk`, set the following parameters in the GUI:
	- "Where is the source code" to `D:\omm-bake-sdk`
	- "Where to build the binaries" to `D:\omm-bake-sdk\build`
...
└───install
    ...
    └───bin
        └─── omm-sdk.dll
```
10. If running `Tests` and CMake have been configured with `-DOMM_TEST_ENABLE_IMAGE_DUMP` a folder with image files will be located in `D:\omm-bake-sdk\build\tests\OmmBakeOutput` and  `D:\omm-bake-sdk\build\tests\OmmBakeOutput_GPU`

## CMake options
`-DOMM_STATIC_LIBRARY=ON` - Will configure the omm-bake-sdk project as either a SHARED (OFF) or STATIC (ON) target in CMake. 
...
`-DOMM_ENABLE_OPENMP=ON` - The project will include OpenMP to enable parallel execution of the CPU baking lib. This is a requirement for ``EnableInternalThreads`` to have an effect.

`-DOMM_INSTALL=ON` - Will configure the ``INSTALL`` solution to produce the library files that can be used in other projects. May need to be disable this when running OMM Bake SDK as submodule.

`-DOMM_DISABLE_INTERPROCEDURAL_OPTIMIZATION=ON` - Will enable LTO on the project via CMAKE_INTERPROCEDURAL_OPTIMIZATION.